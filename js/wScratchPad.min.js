...
function(a) {
	...
	b.prototype = {
		_generate: function() {
			return a.support.canvas ? (this.canvas = document.createElement("canvas"), this.ctx = this.canvas.getContext("2d"), "static" === this.$el.css("position") && this.$el.css("position", "relative"), this.$img = a('<img  class="scratch_img"/>').attr("crossOrigin", "").css({
				position: "absolute",
				width: "100%",
				height: "100%",
				top:"0px",
		...
		reset: function() {
			...
			this.pixels = c * e * d * e,
			this.$img.hide(),
			this.options.bg && ("#" === this.options.bg.charAt(0) ? (this.$el.css("backgroundColor", this.options.bg),this.$img.remove()) : (this.$el.css("backgroundColor", ""), this.$img.attr("src", this.options.bg))),
